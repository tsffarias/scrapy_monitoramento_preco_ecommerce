...
top_10_pages_brands = df.head(500)['brand'].value_counts().nlargest(10)
top_10_pages_brands_table = df.head(500)['brand'].value_counts().sort_values(ascending=False)
...
col1, col2 = st.columns([4, 2])
average_price_by_brand = df.groupby('brand')['new_price'].mean().nlargest(10).sort_values(ascending=False)
...
satisfaction_by_brand = df_non_zero_reviews.groupby('brand')['reviews_rating_number'].mean().nlargest(10).sort_values(ascending=False)
